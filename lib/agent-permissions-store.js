...
export function upsertAgentPermissionByUserId(userId, patch) {
  const items = readPermissions();
  const idx = items.findIndex((item) => item.userId === userId);
  const now = new Date().toISOString();
  ...
  if (idx === -1) {
    const created = {
      id: makeId(),
      userId,
      canEditName: Boolean(patch.canEditName),
      canViewAllTickets: Boolean(patch.canViewAllTickets),
      canViewSpecificUsers: Boolean(patch.canViewSpecificUsers),
      canMarkCashPaid: Boolean(patch.canMarkCashPaid),
      commissionType: patch.commissionType,
      commissionValue: patch.commissionValue,
      createdAt: now,
      updatedAt: now,
    };
    items.push(created);
